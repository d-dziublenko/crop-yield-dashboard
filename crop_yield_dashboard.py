...
def generate_synthetic_data(n_samples=1000):
    ...
    # Nutrient effects
    nutrient_effect = (data['nitrogen'] * 5 + 
                      data['phosphorus'] * 4 + 
                      data['potassium'] * 3)
    ...
    data['yield'] = (yield_base + temp_effect + rain_effect + 
                    nutrient_effect + ph_effect) * crop_effect
...
def train_model(data):
    ...
    for col in data.columns:
        ...
        elif data[col].dtype == 'object' or col.lower() in ['crop_type', 'crop', 'variety']:
    ...
    if yield_column is None:
        st.error("No 'yield' column found in the data. Please ensure your data has a column containing yield values.")
        return None, None, None, None, None, None, None
    
    # One-hot encode categorical variables if any exist
    if categorical_features:
        data_encoded = pd.get_dummies(data, columns=categorical_features, prefix_sep='_')
    else:
        data_encoded = data.copy()
    
    # Separate features and target
    feature_cols = [col for col in data_encoded.columns if col not in [yield_column, 'latitude', 'longitude']]
            ...
            try:
                ...
                with st.expander("📊 Data Preview and Information"):
                    ...
                    if has_yield:
                        ...
                        st.error("❌ No yield column found. Please ensure your data has a column with 'yield' in the name.")
                    
                    # Suggest column mapping if needed
                    if not has_yield:
                        st.info("💡 Tip: Your yield column should be named something like 'yield', 'crop_yield', or 'yield_per_hectare'")
    ...
    if st.button("Train Model", type="primary"):
        if st.session_state.data is not None:
            with st.spinner("Training model..."):
                model, scaler, feature_names, mse, r2, X_train_scaled, X_test_scaled = train_model(st.session_state.data)
...
if st.session_state.model_trained and st.session_state.data is not None:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Overview", "🔮 Predictions", "📈 Model Insights", "🗺️ Geographic View", "📋 Data Analysis"])
    ...
    with tab1:
        ...
        with col1:
            if categorical_cols and yield_col:
                ...
                fig_box = px.box(data, x=cat_col, y=yield_col, 
        ...
        if categorical_cols and yield_col:
            ...
            unique_categories = data[categorical_cols[0]].unique()[:4]  # Limit to 4 categories
            ...
            for cat in unique_categories:
                ...
                if pd.notna(cat_yield):
                    trend_data[str(cat)] = np.random.normal(cat_yield, cat_yield*0.1, len(dates)).cumsum() / len(dates) + cat_yield*0.9
            
            monthly_yields = pd.DataFrame(trend_data)
            
            fig_timeline = px.line(monthly_yields.melt(id_vars='date', var_name='category', value_name='yield'),
                                  x='date', y='yield', color='category',
                                  title=f'Monthly Average Yield by {categorical_cols[0].title()}',
                                  labels={'yield': 'Yield (kg/ha)', 'date': 'Date'})
            st.plotly_chart(fig_timeline, use_container_width=True)
        else:
            st.info("Time series visualization requires categorical data and yield values")
        ...
        if st.button("Predict Yield", type="primary"):
            ...
            for feature_name in st.session_state.feature_names:
                ...
                for key, value in crop_mapping.items():
                    if feature_name.startswith(f"{key}_") and value.lower() in feature_name.lower():
            ...
            explainer = shap.Explainer(st.session_state.model, st.session_state.X_train_scaled)
            shap_values = explainer(input_scaled)
            
            # Create waterfall plot
            st.subheader("Prediction Explanation")
            feature_names_display = [name.replace('crop_type_', '') for name in st.session_state.feature_names]
            ...
            shap_df = shap_df[shap_df['SHAP Value'] != 0].sort_values('SHAP Value', key=abs, ascending=False)
    ...
    with tab3:
        ...
        shap_df = pd.DataFrame(shap_values.values, columns=st.session_state.feature_names)
        
        # Get mean absolute SHAP values for each feature
        mean_shap = shap_df.abs().mean().sort_values(ascending=False).head(10)
        
        fig_shap = px.bar(x=mean_shap.values, y=mean_shap.index,
                         orientation='h',
        ...
        available_features = [f for f in st.session_state.feature_names 
                            if any(keyword in f.lower() for keyword in ['temperature', 'temp', 'rainfall', 'rain', 'ph', 'nitrogen', 'n'])]
        
        if available_features:
            feature_to_plot = st.selectbox("Select feature for partial dependence", available_features)
            
            # Check if the selected feature exists in the original data
            original_feature = None
            for col in data.columns:
                if col.lower() in feature_to_plot.lower() or feature_to_plot.lower() in col.lower():
            ...
            if original_feature and original_feature in data.columns:
                ...
                feature_values = np.linspace(
                    data[original_feature].min(), 
                    data[original_feature].max(), 
                    50
                )
                ...
                for val in feature_values:
                    ...
                    temp_data = data.sample(n=min(50, len(data)), random_state=42).copy()
                    temp_data[original_feature] = val
                    
                    # Process the data similar to training
                    categorical_features = [col for col in temp_data.columns if temp_data[col].dtype == 'object']
                    if categorical_features:
                        temp_encoded = pd.get_dummies(temp_data, columns=categorical_features, prefix_sep='_')
                    else:
                        temp_encoded = temp_data.copy()
                    
                    # Remove target and location columns
                    feature_cols = [col for col in temp_encoded.columns 
                                  if not any(x in col.lower() for x in ['yield', 'latitude', 'longitude'])]
                ...
                fig_pd = px.line(x=feature_values, y=predictions,
                                title=f'Partial Dependence: {original_feature.title()}',
                                labels={'x': original_feature.title(), 'y': 'Predicted Yield (kg/ha)'})
                st.plotly_chart(fig_pd, use_container_width=True)
            else:
                st.info("Please select a feature that exists in the original data for partial dependence plotting.")
        else:
    ...
    with tab4:
        ...
        if has_location:
            ...
            if yield_col:
                ...
                for idx, row in data.iterrows():
                    if idx < 200:  # Limit points for performance
                        ...
                        if label_col:
                            popup_text = f"{label_col.title()}: {row[label_col]}<br>" + popup_text
            ...
            if yield_col:
                ...
                data['region'] = pd.cut(data['latitude'], bins=3, labels=['South', 'Central', 'North'])
                
                if categorical_cols:
                    regional_stats = data.groupby(['region', categorical_cols[0]])[yield_col].agg(['mean', 'std', 'count']).reset_index()
                    
                    fig_regional = px.bar(regional_stats, x=categorical_cols[0], y='mean', color='region',
                                         error_y='std',
                                         title=f'Average Yield by Region and {categorical_cols[0].title()}',
                                         labels={'mean': 'Average Yield (kg/ha)', categorical_cols[0]: categorical_cols[0].title()},
                                         barmode='group')
                else:
                    regional_stats = data.groupby(['region'])[yield_col].agg(['mean', 'std', 'count']).reset_index()
                    
                    fig_regional = px.bar(regional_stats, x='region', y='mean',
                                         error_y='std',
            ...
            st.info("Geographic visualization requires 'latitude' and 'longitude' columns in your data.")
    ...
    with tab5:
        ...
        fig_dist = px.histogram(data, x=feature_to_plot, nbins=30,
        ...
        with col1:
            if st.button("Download Predictions CSV"):
                ...
                data_encoded = pd.get_dummies(data_for_pred, columns=['crop_type'], prefix_sep='_')
                ...
                export_df['yield_difference'] = export_df['predicted_yield'] - export_df['yield']
        ...
        with col2:
            if st.button("Save Model"):
                ...
                st.info("Model saved successfully! (In production, this would save to your model registry)")
...
# Footer
st.markdown("---")
st.markdown("Built with Streamlit, XGBoost, and SHAP | Agricultural Intelligence Platform")